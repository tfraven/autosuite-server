...
import { createBikeSchema, updateBikeSchema, createBikeModelSchema } from '../validation/schemas.js';

const router = Router();
router.use(authenticateToken);

// Helper to format bike with normalized relations flattened for backwards compatibility
function formatBikeResponse(bike: any) {
  if (!bike) return null;
...
router.get('/models/all', async (_req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
...
router.post('/models', requirePermission('MANAGE_BIKES'), validateBody(createBikeModelSchema), async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const { name, brand, engineDisplacement, defaultRetailPrice } = req.body;

    const existing = await prisma.bikeModel.findUnique({ where: { name: name.trim() } });
    if (existing) {
...
router.post('/', requirePermission('MANAGE_BIKES'), validateBody(createBikeSchema), async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    if (!modelId && data.modelName) {
      ...
      let model = await prisma.bikeModel.findUnique({ where: { name: modelNameClean } });
      if (!model) {
    ...
    const fullBike = await prisma.bike.findUnique({
      where: { id: result.id },
    ...
    await logAuditEvent(
      ...
      'CREATE_BIKE',
      'INVENTORY',
      `Registered new bike ${fullBike?.modelName} (Chassis: ${fullBike?.chassisNumber})`,
      req.ip
    );
...
router.put('/:id', requirePermission('MANAGE_BIKES'), validateBody(updateBikeSchema), async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const updated = await prisma.bike.findUnique({
      where: { id },
      include: { model: true, usedDetail: true }
    });
...
router.post('/:id/restore', requirePermission('MANAGE_BIKES'), async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const restored = await prisma.bike.update({
      where: { id },
      data: {
        isDeleted: false,
        deletedAt: null
    ...
    await logAuditEvent(
      ...
      'RESTORE_BIKE',
      'INVENTORY',
      `Restored soft-deleted bike ${restored.modelName} (Chassis: ${restored.chassisNumber})`,
      req.ip
    );
