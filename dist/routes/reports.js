...
router.get('/heatmaps', async (_req, res) => {
    try {
        ...
        const salesHourlyMatrix = [];
        const auditHourlyMatrix = [];
        for (let d = 0; d < 7; d++) {
            for (let h = 0; h < 24; h++) {
        ...
        for (const s of sales) {
            ...
            if (salesHourlyMatrix[idx]) {
                ...
                if (salesHourlyMatrix[idx].count > maxSalesHourlyCount)
                    maxSalesHourlyCount = salesHourlyMatrix[idx].count;
                if (salesHourlyMatrix[idx].revenue > maxSalesHourlyRev)
                    maxSalesHourlyRev = salesHourlyMatrix[idx].revenue;
            }
        ...
        for (const a of auditLogs) {
            ...
            if (auditHourlyMatrix[idx]) {
                ...
                if (auditHourlyMatrix[idx].count > maxAuditHourlyCount)
                    maxAuditHourlyCount = auditHourlyMatrix[idx].count;
            }
